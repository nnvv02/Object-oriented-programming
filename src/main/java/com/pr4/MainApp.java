...
public class MainApp extends Application {
    ...
    private Book createBook(String type, String title, String author, int year, int pages) {
        if ("EBook".equals(type)) {
            return new EBook(title, author, year, pages, BookGenre.GENERAL, "PDF", 1.0);
        }
        if ("PaperBook".equals(type)) {
            return new PaperBook(title, author, year, pages, BookGenre.GENERAL, "Unknown", 1);
        }
        if ("AudioBook".equals(type)) {
            return new AudioBook(title, author, year, pages, BookGenre.GENERAL, 1, "Unknown");
        }
        if ("TextBook".equals(type)) {
            return new TextBook(title, author, year, pages, BookGenre.GENERAL, "General", 1);
        }
        return new GeneralBook(title, author, year, pages, BookGenre.GENERAL);
    }
